# @camera.ui/sdk

[![npm](https://img.shields.io/npm/v/@camera.ui/sdk?label=npm&logo=npm)](https://www.npmjs.com/package/@camera.ui/sdk)
[![PyPI](https://img.shields.io/pypi/v/camera-ui-sdk?label=pypi&logo=pypi&logoColor=white)](https://pypi.org/project/camera-ui-sdk/)
[![Go](https://img.shields.io/github/v/tag/cameraui/sdk?label=go&logo=go&logoColor=white)](https://pkg.go.dev/github.com/cameraui/sdk/go)

camera.ui SDK - A collection of language-specific SDKs for seamless integration with the camera.ui ecosystem. This monorepo contains SDKs for Node.js, Go, and Python, providing developers with the tools they need to interact with camera.ui services and build powerful plugins.

Available for three runtimes:

| Runtime | Package                                  |
| ------- | --------------------------------------- |
| Node    | `@camera.ui/sdk` (`./node`)             |
| Go      | `github.com/cameraui/sdk/go` (`./go`)   |
| Python  | `camera-ui-sdk` (`./python`)            |

---

_Part of the camera.ui ecosystem - A comprehensive camera management solution._
